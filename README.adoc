...
To make a plain old Ruby object functionally composable, include `Core::Composable` from the {core_link} gem which is required by Pipeable for you. Example:
...
The above will allow you to send the following messages: `\#>>`, `#<<`, and `#call`. All you need to do is implement the `#call` message and you get the rest for free. See the {core_link} gem for details.
